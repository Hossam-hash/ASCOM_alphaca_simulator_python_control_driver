...
#####################################
def dome_connection_control_play(dome_base_url, flag):
    """control the connection of dome flag=true make connection true and the oppsite is true """
    url = f"{dome_base_url}"
    data = {'isConn': str(flag)}
    response = requests.put(dome_base_url, data=data)
    status = requests.get(dome_base_url)
    return status.text
    # if status.text:
    #     return 'Dome is connected'
    # else:
...
def connection_dome_status_play(dome_base_url):
    '''Retrieves the connected state of the device if True : connected'''
    dome_connection_tatus_url = f"{dome_base_url}/isConn?"
    return requests.get(dome_connection_tatus_url).text
